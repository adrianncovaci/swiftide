...
/// 2. Chunking creates proper parent-child relationships with origin_id
...
async fn test_chunking_with_cache() {
    ...
    for point in &stored_points.result {
        if let Some(origin_id_value) = point.payload.get("origin_id") {
            let origin_id_str = origin_id_value
                .as_str()
                .expect("origin_id should be a string");
            let origin_id =
                uuid::Uuid::parse_str(origin_id_str).expect("origin_id should be a valid UUID");

            assert!(
                original_node_ids.contains(&origin_id),
                "Chunk origin_id {origin_id} should be one of the original node IDs",
            );
        }
    }
